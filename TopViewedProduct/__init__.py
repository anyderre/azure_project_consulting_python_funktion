import logging
import pandas as pd
from datetime import datetime
from sqlalchemy.engine import URL
from sqlalchemy import create_engine
import sqlalchemy as sa
import json
import os

import azure.functions as func


def main(req: func.HttpRequest) -> func.HttpResponse:
    logging.info('Getting the number of views per product.')

    connection_string = os.environ['AZURE_FUNCTION_CONNECTION_STRING']
    connection_url = URL.create(
        "mssql+pyodbc", query={"odbc_connect": connection_string})
    engine = create_engine(connection_url)
    event_type = req.params.get('event_type')

    with engine.begin() as conn:
        query = "SELECT top 5 productId, count (*) as views FROM [dbo].[events] where eventType = '{}' group by productId order by views desc".format(
            event_type)

        data = pd.read_sql_query(sa.text(query), conn)

        # product_views = data.set_index('productId')

        print(type(data))
        parsed = json.loads(data.to_json(orient='records'))
        return func.HttpResponse(
            json.dumps(parsed),
            status_code=200
        )
